...
export function FileList({ mobile }: FileListProps) {
  const {
  ...
  const columns: TableColumnsType<FileNode> = useMemo(() => {
    if (mobile) {
      return [
        {
          title: '名称',
          ...
          render: (name: string, record: FileNode) => (
            <div className="mv-file-cell">
              <span className="mv-file-cell__icon">{getFileIcon(record.mimeType)}</span>
              <div className="mv-file-cell__body">
                <span className="mv-file-cell__name">{name}</span>
                <span className="mv-file-cell__meta">
                  <span>{formatFileSize(record.size)}</span>
                  <span className="mv-file-cell__dot">·</span>
                  <span>{shortDate(record.modifiedAt)}</span>
                  {record.origin === 'password-book' && <><span className="mv-file-cell__dot">·</span><span>密码本</span></>}
                  {record.origin === 'note' && <><span className="mv-file-cell__dot">·</span><span>笔记</span></>}
                </span>
              </div>
            </div>
          ),
